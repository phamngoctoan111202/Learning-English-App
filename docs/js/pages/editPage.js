...
const EditPage = {
    ...
    copyAiPrompt() {
        ...
        const catValue = categoryRadio ? categoryRadio.value : 'GENERAL';
        const selectedCat = categoryRadio ? categoryRadio.parentElement.textContent.trim() : 'Technical';
        const isTechnical = ['TECHNICAL_MOBILE', 'TECHNICAL_WEB', 'TECHNICAL_BACKEND', 'MOBILE', 'WEB', 'BACKEND'].includes(catValue);

        let promptText = '';
        if (isTechnical) {
            const techTopic = (catValue === 'TECHNICAL_MOBILE' || catValue === 'MOBILE') ? 'Mobile Development' : 'Web & Backend Development';
            promptText = `Hãy tạo cho tôi danh sách các kiến thức ${techTopic} (hoặc chủ đề mà tôi yêu cầu) theo đúng định dạng sau (mỗi từ trên 1 dòng, phân cách bởi dấu gạch đứng |):
...
Vui lòng tạo 10 kiến thức hay và quan trọng nhất. Chỉ trả về dữ liệu kết quả theo đúng định dạng dòng ở trên, không cần thêm lời chào hay giải thích gì khác.`;
            ...
            promptText = `Hãy tạo cho tôi danh sách từ vựng tiếng Anh theo chủ đề: ${selectedCat} (hoặc chủ đề mà tôi yêu cầu) theo đúng định dạng sau (mỗi từ trên 1 dòng, phân cách bởi dấu gạch đứng |):
